...
const COLLECTED = [3200, 4100, 3600, 5200, 4800, 6100];
const PROJECTION = [3500, 3900, 4300, 4700, 5300, 5900];
const KPIS = [
  { key: 'collected', value: 18450, tone: 'text-success-ink' },
  { key: 'receivable', value: 12300, tone: 'text-ink' },
  { key: 'overdue', value: 2150, tone: 'text-danger-ink' },
] as const;
const SAMPLE_AMOUNT = 350;

/** Stylized, decorative preview of the Solvia dashboard with floating notifications. */
        ...
        <GlassCard className="p-4 sm:p-5">
          ...
          <dl className="mt-4 grid grid-cols-3 gap-2 sm:gap-3">
            {KPIS.map((kpi) => (
              ...
              >
                <dt className="truncate text-[11px] text-muted sm:text-xs">
                  {t(`mock.${kpi.key}`)}
                </dt>
                <dd className={cx('mt-0.5 text-sm font-bold tabular-nums sm:text-base', kpi.tone)}>
                  {fmt.compactMoney(kpi.value)}
                </dd>
              </div>
            ))}
          </dl>
          ...
          <div className="mt-3 flex items-center justify-between gap-3 rounded-2xl border border-line/70 bg-surface/80 px-3 py-2.5">
            <div className="flex min-w-0 items-center gap-2.5">
              <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-info-soft text-xs font-semibold text-info-ink">
                RQ
              </span>
              <div className="min-w-0">
                <p className="truncate text-sm font-medium">Rosa Quispe</p>
                <p className="text-xs text-muted tabular-nums">{fmt.money(SAMPLE_AMOUNT)}</p>
              </div>
            </div>
          ...
          >
            ...
            <div>
              ...
              <p className="text-[11px] text-muted tabular-nums">
                {t('mock.toastBody', { name: 'Rosa Q.', amount: fmt.money(SAMPLE_AMOUNT) })}
              </p>
            </div>
          </div>
        </Drift>
